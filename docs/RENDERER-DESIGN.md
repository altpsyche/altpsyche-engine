...
**`Extent` came out narrower than this sketch, and has since gone entirely.** The third case named one axis of the frame for the other axis's size, and nothing needed it: a resource is sized to the frame on both axes or to numbers on both. Because nothing mixed the two, ROADMAP item 71 replaced the per-axis pair `[Extent, Extent]` with one whole-size descriptor — `TransientSize = { scale: number } | { width, height }`, the shape `graph/refs.ts` already carried for a transient. `{ scale: 1 }` is the old `['frame', 'frame']`, `{ width, height }` the old fixed pair, and `{ scale: 0.5 }` a half-resolution target the pair could not say. `graph/refs.ts`'s `sizeAt` is the one place a size becomes pixels, so a resident `TextureResource` and a transient resolve identically. `Extent` the type is gone from the tree.
