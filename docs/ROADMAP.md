...
## Item 20 — two presets draw a different picture on each backend, and the comparison that would have said so did not exist until today — **closed 2026-09-12**
...
3. ~~**`core-texture` and `core-mips` join `SCENE_TIER`** and the diagnostic block comes out.~~
    **Landed on 2026-09-12.** Ten presets are now compared across the two backends on a card, nine of
    them at 0 of 1,440,000 and `core-texture` at 40 at worst 1 against a tolerance of 8. The
    diagnostic block is deleted; the mirror reading it existed to print is now appended to a *failing*
    preset's line, where it costs one comparison over buffers already in hand and answers the first
...
### Done when, verified on 2026-09-12 — item 20 is closed

- **The two backends agree on `core-texture` and `core-mips` to the channel the corpus holds every
  preset to, measured on a card.** `core-texture` 40 of 1,440,000 at worst channel 1 and `core-mips`
  0 at worst 0, against a tolerance of 8, on nvidia / blackwell. From 1,424,706 at worst 235 and
  1,401,861 at worst 128.
- **Both are on the cross-backend list and the comment holding them out is gone.** They are in
  `SCENE_TIER`, the diagnostic block that printed them is deleted, and the header that said they were
  NOT on the list now says why they are.
- **Whatever was wrong is fixed where the decision is, with its reversal and its trigger.** Three
  places: the presentation step and the framebuffer origin in `gpu/webgl2.ts`, the clip-space
  negation at the bake site in `gates/translate.mjs` with `GlslFrameGraph.framebufferOrigin` carrying
  it, and `mipmapFilter` in `gpu/webgpu.ts`. Each carries what would change the answer.
- **`npm test`, `npm run type-check` and `gate:browser` are green, and `gate:card` is re-taken.** 989
  tests over 84 files, type-check clean, `gate:browser` 4 of 4, `gate:card` 34 of 34, `gate:pack` 17
  of 17.

**What it cost, which the plan did not foresee.** Two reverted attempts and three plan corrections.
The first attempt passed 33 of 33 with the frame displayed upside down; the second fixed the
coordinate and broke render-to-texture; between them they proved the fix needed a presentation step
this package did not have. The step list was rewritten twice, one step's text was corrected before it
was built, and step 2e's diagnosis turned out to be wrong in the file that recorded it.

**And the eight presets that were already gated all went to zero**, from 11, 0, 77, 0, 0, 11, 36, 18.
The residuals 11, 36 and 18 had been recorded three times as compiler noise and once as unexplained;
they were the coordinate.

**What would have changed the answer.** If the two pictures had turned out to be correct on both
backends with the disagreement in the generated bytes rather than the sampling, this would have been
a fixture defect and moved to `fixtures/`. It was not: both causes were in the backends.
