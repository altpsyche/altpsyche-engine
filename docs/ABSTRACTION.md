...
| the double models calls rather than lifetimes, so usage and destruction mistakes are invisible to the fast suite      | the engine's own `tests/support/fake-gpu.ts`   | medium   | solved    |
...
**The lifetimes row is marked solved as of 2026-08-25 (item 20).** The recording double now carries a `Lifetimes` ledger beside its trace: a resource used after it was freed is refused by name and a resource never freed is named at a teardown, so the use-after-free and the leak the fast suite was blind to each fail a test. See [ROADMAP.md](ROADMAP.md) item 20.
