...
// Keyed by the resource's index rather than its source name, the way `lib.mjs` has
// keyed generated bytes since item 87; the page reads them back by the same index.
const perdrawBytes = Object.fromEntries([...perdraw.bytes].map(([index, made]) => [index, [...made]]));
...
const gpuResults = await gpuPage.evaluate(
  async ({ graph, broken, state, perdraw }) => {
    ...
    if (!rewriteBackend) {
      ...
      // frame wants them keyed by the resource's index (its handle), which is how
      // `frameOf` reads them now (item 87). Remap through the description's resource
      // order the way `gates/lib.mjs` does.
      const generated = new Map();
      perdraw.description.resources.forEach((resource, index) => {
        const source = 'source' in resource ? resource.source : undefined;
        if (source && perdraw.bytes[index]) generated.set(index, new Uint8Array(perdraw.bytes[index]));
      });
      // The per-draw storage buffer this gate rewrites and reads back, by its handle
      // — the index of the one buffer the per-draw fixture declares. Matched on kind
      // alone: the fixture's buffer is `access: 'read'` (the source binds it as a
      // read-only storage buffer, and `shader-describe` defaults access to 'read'),
      // so a 'read-write' predicate matches nothing and yields -1. A resource carries
      // no `name` since item 87, so the count is what makes the match unambiguous:
      // if the fixture ever declares a second buffer this throws rather than silently
      // rewriting whichever came first.
      const bufferHandles = perdraw.description.resources
        .map((resource, index) => (resource.kind === 'buffer' ? index : -1))
        .filter((index) => index >= 0);
      if (bufferHandles.length !== 1) {
        throw new Error(`core-perdraw should declare exactly one buffer, and declares ${bufferHandles.length}`);
      }
      const copiesHandle = /** @type {import('../graph/handles.js').BufferHandle} */ (bufferHandles[0]);
