  ...
  async ({ graph, broken, state, perdraw }) => {
    ...
    if (!rewriteBackend) {
      ...
      perdraw.description.resources.forEach((resource, index) => {
        ...
        if (source && perdraw.bytes[source]) generated.set(index, new Uint8Array(perdraw.bytes[source]));
      });
      // The per-draw storage buffer this gate rewrites and reads back, by its handle
      // — the index of the one read-write buffer the per-draw fixture declares.
      const copiesHandle = /** @type {import('../graph/handles.js').BufferHandle} */ (
        perdraw.description.resources.findIndex(
          (resource) => resource.kind === 'buffer' && resource.access === 'read-write'
        )
      );
